...
def residualBlock(input_var):
    def f(input):
        ...
        if (not equal_channels):
            x = Conv2D(filters=residual_shape[CHANNEL_AXIS], kernel_size=(1, 1), strides=(stride_width, stride_height),
                       padding="valid")(x)
        y = add([x, l2])
        return y
...
def train(model, epoch=1):
    ...
    history = model.fit(X_train, Y_train, validation_data=(X_test, Y_test), epochs=epoch,verbose=2)
    saveModel(model)
    return history
